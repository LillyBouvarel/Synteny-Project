...
# psl_tropicalis_remanei <- psl_tropicalis_remanei %>%
#   mutate(new_strand = gsub("([[:punct:]])([[:punct:]])", "\\1 \\2", psl_tropicalis_remanei$strand)) %>% 
#   select(-strand) %>% 
#   separate(new_strand, into = c("strand", "strand2"), sep = " ")
...
psl_inopinata_elegans <- read.table(file = "../synteny/psl_inopinata.psl", sep= "\t")
colnames(psl_inopinata_elegans) <- c("matches", "misMatches" ,"repMatches","nCount","qNumInsert","qBaseInsert","tNumInsert","tBaseInsert" ,"strand","scaff","qSize","start","end","seq_id2","tSize","start2","end2","blockCount","blockSizes","qStarts","tStarts")
psl_panamensis_inopinata$seq_id <- paste("panamensis",psl_panamensis_inopinata$scaff, sep = "_")
psl_panamensis_inopinata$seq_id2 <- paste("inopinata",psl_panamensis_inopinata$seq_id2, sep = "_")
# panamensis_inopinata_psl <- panamensis_inopinata_psl %>%
#   mutate(new_strand = gsub("([[:punct:]])([[:punct:]])", "\\1 \\2", panamensis_inopinata_psl$strand)) %>% 
...
psl_inopinata_elegans <- psl_inopinata
```
...
# gene_transcript_ids <- elegans[, c("gene_id", "transcript_id")] %>%  as.data.frame()
# gene_transcript_ortho <- merge(gene_transcript_ids, ortho_elegans, by = "gene_id", all.x = TRUE)
# gene_transcript_ortho <- gene_transcript_ortho %>%  distinct() 
# transcript_ortho <- gene_transcript_ortho[, -1]
# colnames(transcript_ortho)[1] <- "gene_id"
# transcript_ortho <- transcript_ortho %>%  filter(!is.na(gene_id))
...
write.csv(genes_bovis, "../synteny/genes_bovis.csv")
...
genes_elegans <- elegans[,c( "transcript_id", "seq_id", "start", "end", "width", "strand", "Orthogroup")]
genes_elegans <-  genes_elegans  %>%  distinct() %>% mutate(bin_id = "elegans") %>% rename(gene_id = "transcript_id", tSize = "width")
colnames(genes_elegans)[5] <- "length"
colnames(genes_elegans)[7] <- "Orthogroups"

genes <- rbind(genes_bovis, genes_becei, genes_panamensis, genes_inopinata, genes_tropicalis, genes_remanei, genes_latens, genes_tribulationis, genes_briggsae, genes_nigoni)
genes <- rbind(genes_bovis, genes_becei, genes_panamensis, genes_inopinata,genes_tropicalis, genes_remanei)    

colnames(genes) <- c("gene_id", "seq_id", "start", "end", "length", "strand", "Orthogroups", "bin_id")
genes <- rbind(genes, genes_elegans)
```
...
link_2 <- psl_panamensis_becei[,c("start", "end","seq_id", "start2", "end2", "seq_id2") ]
...
link_11 <- psl_bovis_becei[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_12 <- psl_becei_panamensis[,c( "start", "end","seq_id", "start2", "end2", "seq_id2") ]
link_13 <- psl_panamensis_inopinata[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_14 <- psl_inopinata_elegans[, c( "start", "end","seq_id", "start2", "end2", "seq_id2")]
link_15 <- psl_tropicalis[, c( "start", "end","seq_id", "start2", "end2", "seq_id2")]
link_16 <- psl_tropicalis_remanei[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_17 <- psl_remanei_latens[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_18 <- psl_latens_tribulationis[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_19 <- psl_tribulationis_briggsae[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]
link_20 <- psl_briggsae_nigoni[, c("start", "end","seq_id", "start2", "end2", "seq_id2")]

#Data frame with all the links (following the tree) 
link_df <- rbind(link_1, link_2, link_3,link_4,link_6,link_7, link_8, link_9, link_10, link_11, link_12, link_13, link_14, link_15, link_16, link_17, link_18, link_19, link_20)

link_df <- rbind(link_11, link_12, link_13, link_15)
```
...
subset_by_length <- function(min, max){
  sub_genes <- genes %>% filter(between(start, min, max) & between(end, min, max))
  sub_link <- link_df %>% filter(seq_id %in% sub_genes$seq_id & seq_id2 %in% sub_genes$seq_id)
  return(sub_genes)
}
...
select_by_genes <- function(gene){
    sub_genes <- genes %>% filter(gene_id %in% gene)
    sub_link <- link_df %>% filter(seq_id %in% sub_genes$seq_id & seq_id2 %in% sub_genes$seq_id)
}

#subsetting by chromosome on C.elegans
select_by_chromosomes <- function(scaff){
...
select_by_species <- function(species){
...
#subset by length and by chromosome 
sub_genes <- subset_by_length(0,10000)
select_by_chromosomes("V")
...
```{r}
## Gggenomes
geom_1 <- gggenomes(genes = sub_genes, links=  sub_link) +
  geom_gene(aes(fill=Orthogroups))+
  geom_seq()+
  geom_bin_label()+
  geom_link()
geom_1
...
## Gggenomes
geom_2 <- gggenomes(genes = sub_genes, links=  sub_link) +
  geom_gene(aes(fill=Orthogroups))+
  geom_seq()+
  geom_bin_label()+
  geom_seq_label()+
...
```{r}
geom_2 <- gggenomes(genes = sub_gene, links = sub_link) +
   geom_seq()+
   geom_link()+
   geom_gene()+
   geom_bin_label() 
...
geom_3<- gggenomes( genes =list( gene_1, gene_2), links = list(link_1, link_2)) +
   geom_seq()+
   geom_link()+ #first link track 
   geom_link(data = links(link_2))+
   geom_gene()+ #first gene track 
   geom_bin_label() 
geom_3

geom_4 <- gggenomes( genes = sub_gene (CDS), links = sub_link, feats = list(exons, orthogroups)) +
   geom_seq()+ 
   geom_link()+ 
   geom_gene()+
   geom_bin_label()+
   geom_feat()+ #the first feat track (exon) 
   geom_feat(data=feats(orthogroups), colors ="plum3") #use extra feat track by name 
geom_4

geom_5<- gggenomes(genes = sub_gene, links = sub_link) +
  geom_gene() +
  geom_gene_tag(aes(label=ifelse(is.na(gene_id), "<NA>", gene_id)), data=genes(.gene_types = NULL)) +
  geom_seq()+ 
  geom_link()+
  geom_bin_label()
